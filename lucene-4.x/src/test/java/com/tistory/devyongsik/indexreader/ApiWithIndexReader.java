package com.tistory.devyongsik.indexreader;
...
public class ApiWithIndexReader {
	public static void main(String[] args) throws Exception, LockObtainFailedException, IOException {
		...
		IndexReaderContext indexReaderContext = directoryReader.getContext();
		System.out.println("D : " + indexReaderContext.children().size());
		
		TermsEnum termEnum = MultiFields.getTerms(directoryReader, "f").iterator(null);
		
		System.out.println(MultiFields.getTerms(directoryReader, "f").getDocCount()); //3
		System.out.println(MultiFields.getTerms(directoryReader, "f").getSumDocFreq()); //11
		System.out.println(MultiFields.getTerms(directoryReader, "f").getSumTotalTermFreq()); //13
